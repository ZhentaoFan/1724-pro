    ...
    def __init__(self, dic_agent_conf, dic_traffic_env_conf, dic_path, cnt_round,
        ...
        if cnt_round == 0: 
            ...
            try:
                ...
                # print("Error occurs when making samples for inter")
    ...
    def prepare_Xs_Y(self, memory, dic_exp_conf):
        ...
        if dic_exp_conf["PRETRAIN"] or dic_exp_conf["AGGREGATE"]:
            ...
            for i in range(ind_end-1,-1,-1):
                one_sample = memory[i]
                print(one_sample)
                # one_sample_lane_num_vehicle = [np.sum(one_sample[0]['coming_vehicle'][i:i+3]) for i in range(0,len(one_sample[0]['coming_vehicle']),3)]
                one_sample_lane_num_vehicle = one_sample[0]['lane_num_vehicle']
                # print(one_sample_lane_num_vehicle)
