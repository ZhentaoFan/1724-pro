...
class Intersection:
    def __init__(self, inter_id, dic_traffic_env_conf, eng, light_id_dict,path_to_log):
        ...
        # cooresponding exiting lane for entering lanes
        ...
        # print(self.inter_name)
        # print(self.list_entering_lanes)
        # print(self.list_exiting_lanes)
    ...
    def update_current_measurements_map(self, simulator_state, path_to_log,test_flag):
        ...
        # self.dic_lane_waiting_vehicle_count_current_step.append = self.eng.get_vehicle_speed()
        # self.dic_lane_waiting_vehicle_count_current_step = simulator_state["get_lane_waiting_vehicle_count"]
    ...
    def update_current_measurements(self,path_to_log):
        ...
        # print("lane_vehicles",self.dic_lane_vehicle_current_step)
...
# <<<<<<< HEAD
    def log_one_vehicle(self,vehicle,path_to_log):
        # print("log one vehicle")
        # print(self.inter_name.split('_'))
        # print(self.inter_id)
        # inter = self.inter_name.split('_')[1] + self.inter_name.split('_')[0] - 1
    ...
    def log_no_left_vehicles(self, path_to_log):
        for vehicle in self.dic_vehicle_arrive_leave_time.keys():
            self.dic_vehicle_arrive_leave_time[vehicle]["leave_time"] = self.dic_traffic_env_conf["RUN_COUNTS"]
            self.log_one_vehicle(vehicle, path_to_log)
        #log self.dic_vehicle_arrive_leave_time[vehicle]["enter_time"]["leave_time"]
...
#
#     def _get_leaving_vehicles(self):
#         leaving_distribution = []
#
#         lane_vid_mapping_dict = self.eng.get_lane_vehicles()
#         vid_distance_mapping_dict = self.eng.get_vehicle_distance()
#
#         bins = np.linspace(0, 300, 4).tolist()
#
#         for lane in self.list_exiting_lanes:
#             leaving_vehicle_position = []
#             vehicle_position_lane = lane_vid_mapping_dict[lane]
#             for vehicle in vehicle_position_lane:
#                 leaving_vehicle_position.append(vid_distance_mapping_dict[vehicle])
#             leaving_distribution.extend(self.lane_position_mapper(leaving_vehicle_position, bins))
# =======
    ...
    def get_reward(self, dic_reward_info):
        ...
        dic_reward["sum_lane_num_vehicle"] = np.sum(self.dic_feature["lane_num_vehicle"])
        dic_reward["sum_delta_lane_num_vehicle"] = np.absolute(np.sum(self.dic_feature["delta_lane_num_vehicle"]))
...
class AnonEnv:
    ...
    def reset(self):
        ...
        config_path = config['dir'] + self.dic_traffic_env_conf["TRAFFIC_FILE"][:-5] + '_config.json'
        with open(config_path, 'w') as fp:
            json.dump(config, fp)

        self.eng = cityflow.Engine(config_path, self.dic_traffic_env_conf["THREADNUM"])

        # self.load_roadnet(self.dic_traffic_env_conf["ROADNET_FILE"])

        # if self.dic_traffic_env_conf["SEPERATE_TEST"]:
        #     self.load_flow(self.dic_traffic_env_conf["SEPERATE_TRAFFIC"])
        # else:
        # self.load_flow(self.dic_traffic_env_conf["TRAFFIC_FILE"])
        ...
        if self.dic_traffic_env_conf["FAST_COMPUTE"]:
            ...
            # print(self.system_states["get_lane_vehicles"])
    ...
    def reset_test(self):
        ...
        config = {
            ...
            "dir": "data/template_lsr/4_4/",
            "roadnetFile": self.dic_traffic_env_conf["ROADNET_FILE"],
            "flowFile": self.dic_traffic_env_conf["TRAFFIC_SEPARATE"],
            "rlTrafficLight": self.dic_traffic_env_conf["RLTRAFFICLIGHT"],
            "saveReplay": self.dic_traffic_env_conf["SAVEREPLAY"],
        }
        config_path = config['dir'] + self.dic_traffic_env_conf["TRAFFIC_FILE"][:-5] + '_config.json'
        ...
        # initialize intersections (grid)
        self.list_intersection = [Intersection((i+1, j+1), self.dic_traffic_env_conf, self.eng,
                                               self.traffic_light_node_dict["intersection_{0}_{1}".format(i+1, j+1)],path_to_log=self.path_to_log)
                                  for i in range(self.dic_traffic_env_conf["NUM_ROW"])
                                  for j in range(self.dic_traffic_env_conf["NUM_COL"])]
        self.list_inter_log = [[] for i in range(self.dic_traffic_env_conf["NUM_ROW"] *
                                                 self.dic_traffic_env_conf["NUM_COL"])]
        ...
        self.system_states["num_total_veh"] = np.sum([len(self.system_states["get_lane_vehicles"][lane]) for lane in self.list_lanes])
            ...
            # self.system_states["num_total_veh"] = np.sum([self.system_states["get_lane_vehicles"][lane] for lane in self.list_lanes])
    ...
    def log_no_left_vehicles(self):
        for inter in self.list_intersection:
            inter.log_no_left_vehicles(self.path_to_log)
...
# >>>>>>> ana_simulator
        ...
        for i in range(self.dic_traffic_env_conf["MIN_ACTION_TIME"]):
            ...
            if self.dic_traffic_env_conf['DEBUG']:
                ...
                if i == 0:
                    ...
                    #       .format(instant_time,
                    #               str([before_action_feature[j]["cur_phase"] for j in
                    #                    range(len(before_action_feature))]),
                    #               str([before_action_feature[j]["time_this_phase"] for j in
                    #                    range(len(before_action_feature))]),
                    #               str(list_action_in_sec_display))
                    #       )
    ...
    def get_state(self):
        ...
        # print(list_state[0])
        # sys.exit(-1)
        # list state is {state_feature_name: self.dic_feature[state_feature_name]}
    ...
    #
    # @staticmethod
    # def batch_log_process(inter_ind, path_to_log, intersection, dic_vehicle):
    #     print("Batch log for inter ",inter_ind)
    #     path_to_log_file = os.path.join(path_to_log, "vehicle_inter_{0}.csv".format(inter_ind))
    ...
    #         dic_vehicle = self.list_intersection[i].get_dic_vehicle_arrive_leave_time()
    #         agent_r = pool.apply_async(self.batch_log_process, (i, self.path_to_log, self.list_intersection[i], dic_vehicle))
    #         agents_r.append(agent_r)
    #
...
# >>>>>>> ana_simulator
    ...
    def bulk_log(self):
        ...
        #self.eng.print_log(os.path.join("data", "frontend", "web", "roadnet_1_1.json"),
        #                         os.path.join("data", "frontend", "web", "replay_1_1_%s.txt"%vol))
        self.eng.set_replay_file(os.path.join(self.path_to_log, "replay_1_1_%s.txt"%vol))
    ...
    def log_first_vehicle(self):
        ...
        #    pos_2, speed_2 = inter._get_vehicle_info(veh_id_2)
        #    if pos and speed:
        #        print("%f, %f, %f"%(self.get_current_time(), pos, speed), file=open(os.path.join(self.path_to_log, "first_vehicle_info.txt"), "a"))
        #    if pos_2 and speed_2:
        #        print("%f, %f, %f" % (self.get_current_time(), pos_2, speed_2),
        #              file=open(os.path.join(self.path_to_log, "first_vehicle_info_2.txt"), "a"))

                #print(self.get_current_time(), pos, speed)
                #print(str(self.get_current_time()) + ", " + ", " + list_to_str(inter._get_lane_vehicle_position([lane])[0]),
                #      file=open(os.path.join(self.path_to_log, "lane_vehicle_position_%s.txt"%dic_lane_map[lane]), "a"))
