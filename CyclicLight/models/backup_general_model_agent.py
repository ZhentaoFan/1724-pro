...
class GeneralAgent(NetworkAgent):
    ...
    def choose_action(self, states):
        ...
        print('np.array(cur_phase_info): ', np.array(cur_phase_info))
    ...
    def train_network(self, memory):
        ...
        for epoch in range(epochs):
            ...
            for ba in range(int(num_batch*percent)):
                ...
                # print('batch_Xs1[0]: ', batch_Xs1[0][0,:,:])
                print('batch_Xs1[1]: ', batch_Xs1[1][0,:])
                print('batch_Xs2[1]: ', batch_Xs2[1][0,:])
                # # print('batch_r: ', batch_r)
                # print('batch_a: ', batch_a[0])
                print(self.dic_traffic_env_conf["PHASE"][batch_a[0]+1] == batch_Xs2[1][0,:])
                # forward
    ...
    # penalize the non-cyclic action
    def train_network3(self, memory):
        ...
        for i in range(len(_action)):
            if(i > 0):
                if self.isCyclic(_action[i-1], _action[i]):
                    print("cyclic")
                    print(_action[i])
                elif _action[i-1] == _action[i]:
                    print("same")
                else:
                    print("not cyclic")
    ...
    def isCyclic(self, prevAction, curAction):
        ...
        #     return True
        if (prevAction + 1 % self.num_phases == curAction):
            return True
        else:
            return False

    
    def train_network4(self, memory):
        _state, _action, _next_state, _reward = self.prepare_samples(memory)
